...
func GetImageTestCodeListHandler(w http.ResponseWriter, r *http.Request) {
	util.EnableCors(&w)
	testCodeList, tagsList := sql.GetImageTestCodeInfo()
	fmt.Print("image testcode list: ")
	fmt.Println(testCodeList, tagsList)
	ressult := struct {
		TestCode []string `json:"testcode"`
		Tags     []string `json:"tags"`
	}{
...
func GetImageListFromTagHandler(w http.ResponseWriter, r *http.Request) {
	fmt.Println(r.Method)
	util.EnableCors(&w)
	tags := r.URL.Query()["tag[]"]
	fmt.Print("tag data is: ")
	...
	for _, tag := range tags {
		iamges, _ := sql.GetImageListFromTag(tag)
		for _, iamge := range iamges {
...
func GetImageTestCodeHandler(w http.ResponseWriter, r *http.Request) {
	// util.EnableCors(&w)
	// //TODO: 이거 뭐임?
	// //testCode := r.URL.Query().Get("testcode")
	// testCodeInfo := sql.GetTestCode(testCode)
